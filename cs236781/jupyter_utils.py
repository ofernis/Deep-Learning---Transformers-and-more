...
def nbconvert(
    ...
    if timeout_sec is not None:
        # The time to wait (in seconds) for output from executions (per cell).
        args.append(f"--ExecutePreprocessor.timeout={timeout_sec}")
        # IOPub timeout means cell has produced no output for a certain amount
        # of time
        args.append(f"--ExecutePreprocessor.iopub_timeout={timeout_sec}")
    args.append(nb_path)
    ...
    print(
        f">> Finished nbconvert on notebook {nb_path}, "
...
def show_video_in_notebook(
    video_path, width=500, height="auto", autoplay=True, embed=True
):
    """
    ...
    raw_html = (
        f'<video src="{src_str}" controls {autoplay} '
